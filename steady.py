...
vnorm = fe.sqrt(fe.dot(u, u) + EPSILON)
#tau = ((4.0*(nu + nu_tv)/h**2)**2 + EPSILON)**(-0.5)
#tau = (nu + nu_tv) / h **2
tau = ((2.0*vnorm/h)**2 + 9 * (4.0*(nu)/h**2)**2 + EPSILON)**(-0.5)
res = fe.grad(u)*u - (nu + nu_tv) * fe.div(fe.grad(u)) + fe.grad(p) - b
Q = fe.dot(u, fe.grad(v))  -  fe.grad(q)

if STAB: 
   weakForm += fe.inner(tau * Q, res) * fe.dx
